  ...
  async sendInvitations(
    ...
    for (const invitation of invitationResults) {
      if (invitation.status === 'fulfilled') {
        ...
        const joinTeamMsg = msg`Join your team on Octave CRM AI`;
        const i18n = this.i18nService.getI18nInstance(sender.locale);
        const subject = i18n._(joinTeamMsg);

        await this.emailService.send({
          from: `${sender.name.firstName} ${sender.name.lastName} (via Octave CRM AI) <${this.twentyConfigService.get('EMAIL_FROM_ADDRESS')}>`,
